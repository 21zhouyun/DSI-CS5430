...
def main():
    ...
    # # Now save this set to a JSON file
    # with open('data/restricted_id_set.json', 'w') as json_file:
    #     json.dump(restricted_id_set, json_file)

    with open('data/restricted_id_set.json', 'r') as json_file:
        restricted_id_set = json.load(json_file)
    ...
    def restrict_decode_vocab(batch_idx, sent_so_far):
        ...
        if decoded_tokens[-1] == tokenizer.pad_token:
            ...
            if not valid_next_tokens:
                # Default to end of sequence token if no valid continuation
                valid_next_token_ids = [tokenizer.eos_token_id]
